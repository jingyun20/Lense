...
Lense <- function(seurat_obj, nfeatures_vals = NULL, pc_vals = c(5, 20), resolution = 0.3,
  ...
  output_dir <- file.path(getwd(), "lense_umaps")
  ...
  if (ncol(seurat_obj) == 0) stop("All cells removed during filtering! Check input Seurat object.")

  message("Using GPT-4o for UMAP visual comparison.")

  gpt_compare_umap <- function(img1_path, img2_path, api_key) {
    if (!file.exists(img1_path) || !file.exists(img2_path)) {
      stop("Image paths invalid: ", img1_path, " or ", img2_path)
    }
