...
#' @param api_key Your OpenAI API key. If not provided, the function will use the environment variable `OPENAI_API_KEY`.
#'
#' @return A named list:
#' \describe{
...
gpt_compare_umap <- function(img1_path, img2_path, api_key = Sys.getenv("OPENAI_API_KEY")) {
      ...
      list(
        ...
        content = list(
          list(type = "text", text = paste(
            ...
            "Please respond ONLY with: 1 or 2.",
            "No other text. Which image shows better cluster separation and boundary clarity?"
          )),
  ...
  message("GPT-4o raw reply: ", reply)
